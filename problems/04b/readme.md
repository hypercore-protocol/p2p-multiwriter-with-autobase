...
`createRebasedIndex` behaves differently depending on if the Hypercores it's given are readable (meaning coming from remote peers) or writable (local Hypercores). If you give it an Array of readable Hypercores, those will be treated as remote indexes. If the Hypercores are writable, they will be treated as local indexes, and the complete causal stream will be rebased into them.
